...
# Usage reports
#
# Graph Reports API data arrives as strings from CSV. Numeric columns are
# cast to long here in the silver layer. SCD Type 1 keeps the latest
# snapshot per site/team; bronze retains all historical snapshots.
# ============================================================================
...
@dlt.view(name="v_report_spo_site_usage")
def v_report_spo_site_usage():
    return (
        spark.readStream.table("matoolkit_analytics.bronze.report_spo_site_usage")
        .select(
            concat_ws("_", col("_tenant_key"), col("site_id")).alias("_scd_key"),
            col("_tenant_key").alias("tenant_key"),
            col("report_refresh_date"),
            col("site_id"),
            col("site_url"),
            col("owner_display_name"),
            col("is_deleted"),
            col("last_activity_date"),
            col("file_count").cast("long").alias("file_count"),
            col("active_file_count").cast("long").alias("active_file_count"),
            col("page_view_count").cast("long").alias("page_view_count"),
            col("visited_page_count").cast("long").alias("visited_page_count"),
            col("storage_used_byte").cast("long").alias("storage_used_byte"),
            col("storage_allocated_byte").cast("long").alias("storage_allocated_byte"),
            col("root_web_template"),
            col("owner_principal_name"),
            col("report_period"),
            col("_source_file"),
            col("_dlt_ingested_at"),
        )
...
dlt.create_streaming_table(
    name="report_spo_site_usage",
    comment="SharePoint site usage reports (activity, storage, page views) across all tenants",
    table_properties=SILVER_TABLE_PROPERTIES,
)

dlt.apply_changes(
    target="report_spo_site_usage",
    source="v_report_spo_site_usage",
    keys=["_scd_key"],
    sequence_by=col("_dlt_ingested_at"),
    stored_as_scd_type=1,
)
...
# --- Teams Team Activity Report ---


@dlt.view(name="v_report_teams_team_activity")
def v_report_teams_team_activity():
    return (
        spark.readStream.table("matoolkit_analytics.bronze.report_teams_team_activity")
        .select(
            concat_ws("_", col("_tenant_key"), col("team_id")).alias("_scd_key"),
            col("_tenant_key").alias("tenant_key"),
            col("report_refresh_date"),
            col("team_name"),
            col("team_id"),
            ...
            col("last_activity_date"),
            col("report_period"),
            col("active_users").cast("long").alias("active_users"),
            col("active_channels").cast("long").alias("active_channels"),
            col("guests").cast("long").alias("guests"),
            col("reactions").cast("long").alias("reactions"),
            col("meetings_organized").cast("long").alias("meetings_organized"),
            col("post_messages").cast("long").alias("post_messages"),
            col("reply_messages").cast("long").alias("reply_messages"),
            col("channel_messages").cast("long").alias("channel_messages"),
            col("urgent_messages").cast("long").alias("urgent_messages"),
            col("mentions").cast("long").alias("mentions"),
            col("active_shared_channels").cast("long").alias("active_shared_channels"),
            col("active_external_users").cast("long").alias("active_external_users"),
            col("_source_file"),
            col("_dlt_ingested_at"),
        )
    )
...
dlt.create_streaming_table(
    ...
    comment="Teams team activity reports (messages, meetings, users) across all tenants",
    table_properties=SILVER_TABLE_PROPERTIES,
)

dlt.apply_changes(
    target="report_teams_team_activity",
    source="v_report_teams_team_activity",
    keys=["_scd_key"],
    sequence_by=col("_dlt_ingested_at"),
    stored_as_scd_type=1,
)
