...
# Usage reports
#
# Microsoft Graph Reports APIs return CSV data which the entity modules
# convert to JSONL with snake_case column names. All fields arrive as strings;
# numeric casting happens in the silver layer.
# ============================================================================

_REPORT_SPO_SITE_USAGE_SCHEMA = StructType(
    [
        StructField("report_refresh_date", StringType()),
        StructField("site_id", StringType()),
        StructField("site_url", StringType()),
        StructField("owner_display_name", StringType()),
        StructField("is_deleted", StringType()),
        StructField("last_activity_date", StringType()),
        StructField("file_count", StringType()),
        StructField("active_file_count", StringType()),
        StructField("page_view_count", StringType()),
        StructField("visited_page_count", StringType()),
        StructField("storage_used_byte", StringType()),
        StructField("storage_allocated_byte", StringType()),
        StructField("root_web_template", StringType()),
        StructField("owner_principal_name", StringType()),
        StructField("report_period", StringType()),
    ]
)
...
@dlt.table(
    ...
    comment="Raw SharePoint site usage detail reports from all tenants",
    table_properties=BRONZE_TABLE_PROPERTIES,
)
@dlt.expect("valid_record", "site_id IS NOT NULL")
def report_spo_site_usage():
    return _read_landing(
        "enrichment",
        "report_spo_site_usage",
        schema=_REPORT_SPO_SITE_USAGE_SCHEMA,
    )


_REPORT_TEAMS_TEAM_ACTIVITY_SCHEMA = StructType(
    [
        StructField("report_refresh_date", StringType()),
        ...
        StructField("last_activity_date", StringType()),
        StructField("report_period", StringType()),
        StructField("active_users", StringType()),
        StructField("active_channels", StringType()),
        StructField("guests", StringType()),
        StructField("reactions", StringType()),
        StructField("meetings_organized", StringType()),
        StructField("post_messages", StringType()),
        StructField("reply_messages", StringType()),
        StructField("channel_messages", StringType()),
        StructField("urgent_messages", StringType()),
        StructField("mentions", StringType()),
        StructField("active_shared_channels", StringType()),
        StructField("active_external_users", StringType()),
    ]
)
...
@dlt.table(
    ...
    comment="Raw Teams team activity detail reports from all tenants",
    table_properties=BRONZE_TABLE_PROPERTIES,
)
@dlt.expect("valid_record", "team_id IS NOT NULL")
def report_teams_team_activity():
    return _read_landing(
        "enrichment",
        "report_teams_team_activity",
        schema=_REPORT_TEAMS_TEAM_ACTIVITY_SCHEMA,
    )
