function Get-StorageToken {
    if ($script:StorageAuthMethod -eq 'service_principal') {
        ...
        $cert = [System.Security.Cryptography.X509Certificates.X509Certificate2]::new(
            $script:StorageCertBytes, [string]::Empty,
        ...
        $payload = @{
            aud = "https://login.microsoftonline.com/$($script:StorageSpTenantId)/oauth2/v2.0/token"
            iss = $script:StorageSpClientId
            sub = $script:StorageSpClientId
        ...
        $tokenResponse = Invoke-RestMethod `
            -Uri "https://login.microsoftonline.com/$($script:StorageSpTenantId)/oauth2/v2.0/token" `
            -Method POST `
            -ContentType 'application/x-www-form-urlencoded' `
            -Body @{
                client_id             = $script:StorageSpClientId
...
Export-ModuleMember -Function Write-ToAdlsRest, Get-StorageToken
