function Get-EntityConfig {
    return @{
        ...
        Phase2       = $true
        ApiSource    = 'graph'
        OutputFile   = 'entra_sp_claims_mapping_policies'
        DetailType   = 'claims_policies'
    }
}
...
function Invoke-Phase2 {
    ...
    try {
        ...
        if ($failedRunspaces -eq $PoolSize) {
            throw "All runspaces failed to authenticate. Cannot enumerate SP claims mapping policies."
        }
            ...
            $ps = [PowerShell]::Create().AddScript({
                ...
                try {
                    foreach ($spId in $SpIds) {
                        ...
                        while (-not $spDone) {
                            $attempt++
                            $uri = "/v1.0/servicePrincipals/$spId/claimsMappingPolicies"
