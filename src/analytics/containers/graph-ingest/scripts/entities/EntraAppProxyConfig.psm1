...
function Invoke-Phase2 {
    ...
    try {
        ...
        if ($failedRunspaces -eq $PoolSize) {
            throw "All runspaces failed to authenticate. Cannot enumerate app proxy configuration."
        }
        ...
        for ($chunkIndex = 0; $chunkIndex -lt $slices.Count; $chunkIndex++) {
            $ps = [PowerShell]::Create().AddScript({
                param($AppIds, $OutputDir, $ChunkNum, $RunId)
                ...
                try {
                    foreach ($appId in $AppIds) {
                        ...
                        while (-not $appDone) {
                            ...
                            $uri = "/beta/applications/$appId?`$select=id,displayName,onPremisesPublishing"

                            try {
                                $response = Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop

                                # Only write records where App Proxy is configured
                                ...
                                if ($attempt -ge $MaxRetries) {
                                    $errors.Add("app=$appId attempt=${attempt}: $($class.Message)")
                                    $skipped++
                                    $appDone = $true
                                    continue
                                }
