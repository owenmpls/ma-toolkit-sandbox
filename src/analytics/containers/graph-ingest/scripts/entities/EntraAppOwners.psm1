...
function Invoke-Phase2 {
    ...
    try {
        ...
        if ($failedRunspaces -eq $PoolSize) {
            throw "All runspaces failed to authenticate. Cannot enumerate application owners."
        }
            ...
            $ps = [PowerShell]::Create().AddScript({
                ...
                try {
                    foreach ($appId in $AppIds) {
                        ...
                        while (-not $appDone) {
                            $attempt++
                            $uri = "/v1.0/applications/$appId/owners?`$select=id,displayName,userPrincipalName,mail&`$top=999"

                            try {
                                do {
                                    $response = Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop
                                    foreach ($owner in $response.value) {
                                        $owner['applicationId'] = $appId
