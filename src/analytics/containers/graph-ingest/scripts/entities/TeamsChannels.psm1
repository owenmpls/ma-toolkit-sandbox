...
function Invoke-Phase2 {
    ...
    try {
        ...
        if ($failedRunspaces -eq $PoolSize) {
            throw "All runspaces failed to authenticate. Cannot enumerate team channels."
        }
        ...
        for ($chunkIndex = 0; $chunkIndex -lt $slices.Count; $chunkIndex++) {
            $ps = [PowerShell]::Create().AddScript({
                param($TeamIds, $OutputDir, $ChunkNum, $RunId)
                ...
                try {
                    foreach ($teamId in $TeamIds) {
                        ...
                        while (-not $teamDone) {
                            $attempt++
                            $uri = "/v1.0/teams/$teamId/channels?`$select=id,displayName,description,membershipType,createdDateTime,webUrl,email,isArchived"
                            ...
                            try {
                                do {
                                    ...
                                    foreach ($channel in $response.value) {
                                        ...
                                        $writer.WriteLine(($channel | ConvertTo-Json -Compress -Depth 5))
                                        $processed++
                                    }
                                    $uri = $response['@odata.nextLink']
                                } while ($uri)
                                ...
                                if ($attempt -ge $MaxRetries) {
                                    $errors.Add("team=$teamId attempt=${attempt}: $($class.Message)")
                                    $skipped++
                                    $teamDone = $true
                                    continue
                                }
