...
function Invoke-Phase2 {
    ...
    return Invoke-EntityPhase2 -EntityName 'entra_sp_owners' `
...
$uri = "/v1.0/servicePrincipals/$ItemId/owners?`$select=id,displayName,userPrincipalName,mail&`$top=999"
do {
    $response = Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop
    foreach ($owner in $response.value) { $owner['servicePrincipalId'] = $ItemId; $records.Add($owner) }
    $uri = $response['@odata.nextLink']
} while ($uri)
