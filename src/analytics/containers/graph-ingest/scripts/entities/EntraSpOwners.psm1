...
function Invoke-Phase2 {
    ...
    try {
        ...
        if ($failedRunspaces -eq $PoolSize) {
            throw "All runspaces failed to authenticate. Cannot enumerate service principal owners."
        }
            ...
            $ps = [PowerShell]::Create().AddScript({
                ...
                try {
                    foreach ($spId in $SpIds) {
                        ...
                        while (-not $spDone) {
                            $attempt++
                            $uri = "/v1.0/servicePrincipals/$spId/owners?`$select=id,displayName,userPrincipalName,mail&`$top=999"

                            try {
                                do {
                                    $response = Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop
                                    foreach ($owner in $response.value) {
                                        $owner['servicePrincipalId'] = $spId
