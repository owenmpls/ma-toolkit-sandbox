...
public class SqlQueryClient : ISqlQueryClient
{
    private readonly IConfiguration _config;
    private readonly ILogger<SqlQueryClient> _logger;
    private readonly QueryClientSettings _settings;

    public SqlQueryClient(IConfiguration config, ILogger<SqlQueryClient> logger, IOptions<QueryClientSettings> settings)
    {
        _config = config;
        _logger = logger;
    ...
    public async Task<DataTable> ExecuteQueryAsync(string connectionEnvVar, string query)
    {
        ...
        _logger.LogInformation("SQL query returned {RowCount} rows", dataTable.Rows.Count);
        return dataTable;
    }
}
