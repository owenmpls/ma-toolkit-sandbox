<#
.SYNOPSIS
    Runspace pool management for the Hybrid Worker.
.DESCRIPTION
...
function Initialize-RunspacePool {
    ...
    for ($i = 0; $i -lt $Config.MaxParallelism; $i++) {
        ...
        $authScript = Get-RunspaceAuthScriptBlock -TenantId $Config.TargetTenantId -AppId $Config.TargetAppId -Organization $Config.TargetOrganization -CertificateBytes $certBytes

        $ps.AddScript($authScript).AddParameters(@{
            TenantId         = $Config.TargetTenantId
            AppId            = $Config.TargetAppId
