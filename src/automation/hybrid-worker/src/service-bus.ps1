<#
.SYNOPSIS
    Azure Service Bus integration for the Hybrid Worker.
.DESCRIPTION
    Manages Service Bus client, subscription, message receiving, and result sending
    using the Azure.Messaging.ServiceBus .NET SDK. Authenticates via
...
function New-ServiceBusClient {
    <#
    .SYNOPSIS
        Creates a Service Bus client using ClientCertificateCredential (SP + cert).
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    # ClientCertificateCredential instead of ManagedIdentityCredential
